...
describe("Ownership", function() {
    ...
    it("Deploy should set the owner correctly (gas estimate)", async function() {
        const [owner] = await ethers.getSigners();

        expect(await deployment.owner()).to.equal(owner.address);
    });

    it("Can transfer the owner (gas estimate)", async function() {
    ...
    it("Can renounce the owner (gas estimate)", async function() {
